...
const PlaneContainer = ({ flights, goFlights, returnFlights, setFlights, setGoFlights, setReturnFlights }) => {
  ...
  const renderContent = () => {
    ...
    if (flights.length > 0) {
      return <div><button className="reserve-btn" onClick={() => handleBookingFlights(false)}>Đặt vé</button></div>
    } else if (goFlights.length > 0 || returnFlights.length > 0) {
      return <div><button className="reserve-btn" onClick={() => handleBookingFlights(true)}>Đặt vé</button></div>
    }
    else {
      return (
